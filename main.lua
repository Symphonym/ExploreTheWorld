...
do
	local inspectFrameHandler = CreateFrame("Frame", nil, UIParent)
	inspectFrameHandler:RegisterEvent("INSPECT_READY")
	inspectFrameHandler:RegisterEvent("CHAT_MSG_ADDON")
	inspectFrameHandler:SetScript("OnEvent", function(self, event, ...)
		if(event == "INSPECT_READY") then

			if(ETW_Frame.inspectFrame == nil) then
				local frame = CreateFrame("Frame", "ETW_InspectFrame", InspectPaperDollFrame, "InsetFrameTemplate3")
				frame:SetWidth(230);
				frame:SetHeight(24);
				frame:SetPoint("TOP", 10, -37)

				frame.text = frame:CreateFontString("ETW_InspectFrameText", "ARTWORK", "GameFontNormalSmall")
				frame.text:SetPoint("LEFT", 5, 0)

				ETW_Frame.inspectFrame = frame
			end

			ETW_Frame.inspectFrame.text:SetText("ETW Rank:" .. ETW_Utility:RGBToStringColor(0.6, 0, 0) .. " Unknown |r(??/??)")
		...
		elseif(event == "CHAT_MSG_ADDON") then
			...
			if(prefix == ETW_ADDONMSG_PREFIX) then
				...
				-- Receive data
				elseif(messageCount == 3 and messageList[1] == ETW_ADDONMSG_INSPECT_REPORT) then

					local senderCompleted = tonumber(messageList[2])
					local senderTotalQs = tonumber(messageList[3])

					if(senderCompleted and senderTotalQs) then
						ETW_Frame.inspectFrame.text:SetText(
							"ETW Rank: " .. ETW_Utility:RGBToStringColor(0.6, (senderCompleted / senderTotalQs), 0) ..ETW_GetQuestionRank((senderCompleted/senderTotalQs) * 100).."|r ("..senderCompleted.."/"..senderTotalQs..")")
					end
				end
			end
			--[[local prefix, sentMessage, channel, sender = ...
			...
			if(messageCount == 5 and messageList[1] == ETW_ADDONMSG_LINK and not IsIgnored(sender)) then
				...
				local questionID = tonumber(messageList[2])
				local senderRank = ETW_ETW_GetQuestionRank((senderCompleted/senderTotalQs) * 100)(messageList[4]/messageList[3] * 100)
				local senderVersion = messageList[5]
	...
	ETW_Frame.inspectHandler = inspectFrameHandler
